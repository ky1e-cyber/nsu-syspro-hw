...
// who let the C in >:(
void inputArray(FILE* stream, TYPE* v, size_t size) {
    for (size_t i = 0; i <= size; i++) 
        fscanf(stream, FORMAT, v + i);
}
...
int main(int argc, char** argv) {
    ...
    const size_t repeats = atol(argv[3]);
    if (length == 0) return 2;

    std::vector<TYPE> v(length);
    FILE* file = fopen(file_path, "r");
    inputArray(file, &v[0], length);
    fclose(file);

    timespec start, end;
    double timeProduct = 1;
    ...
    printf("{\"geometric_mean\": %.4f}\n", std::pow(timeProduct, 1.0 / repeats));
}
