...
class Codec:
    ...
    def serialize(self, root: Optional[TreeNode]) -> str:
        ...
        def put_subtree(root: Optional[TreeNode], ind: int):
            if not (root is None):
                lst[ind] = str(root.val)
        ...
        lst = [Codec.NULL_TOKEN for _ in range(2 ** tree_height(root) - 1)]
        put_subtree(root, 0)
...
tree = codec.deserialize(ser_tree)
