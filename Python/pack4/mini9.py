...
def format_table(benchmarks: List[str], algos: List[str], results: List[List[float]]):
    ...
    results_str: List[str] = [list(map(lambda n: str(round(n, 2)), seq)) for seq in results]
    ...
    column_lens: List[int] = list(
        map(
            ...
            [[col_name] + [results_str[j][i] for j in range(len(results_str))] 
    ...
    print(
        "\n".join(
            ...
            [get_format_row([bench] + res, column_lens) for bench, res in zip(benchmarks, results_str)]
        )
    )
## surprisingly wackiest thing i ever wrote
